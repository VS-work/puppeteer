...
class Launcher {
  ...
  async launch(options = {}) {
    ...
    console.timeEnd('puppeteer-spawn');
    ...
    try {
      if (!usePipe) {
        console.time('puppeteer-pipe');
        const browserWSEndpoint = await waitForWSEndpoint(chromeProcess, timeout, this._preferredRevision);
        console.timeEnd('puppeteer-pipe');
        console.time('puppeteer-socket');
        const transport = await WebSocketTransport.create(browserWSEndpoint);
        console.timeEnd('puppeteer-socket');
        console.time('puppeteer-connection');
        connection = new Connection(browserWSEndpoint, transport, slowMo);
        console.timeEnd('puppeteer-connection');
      } else {
        console.time('puppeteer-pipe2');
        ...
        console.timeEnd('puppeteer-pipe2');
        console.time('puppeteer-connection2');
        connection = new Connection('', transport, slowMo);
        console.timeEnd('puppeteer-connection2');

      }
      console.time('puppeteer-browser');
      const browser = await Browser.create(connection, [], ignoreHTTPSErrors, defaultViewport, chromeProcess, gracefullyCloseChrome);
      console.timeEnd('puppeteer-browser');
      console.time('puppeteer-ensureInitialPage');
      await ensureInitialPage(browser);
      console.timeEnd('puppeteer-ensureInitialPage');
      return browser;
    } catch (e) {
      killChrome();
      throw e;
